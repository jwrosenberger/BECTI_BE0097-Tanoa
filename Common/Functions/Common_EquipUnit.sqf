...
if (_unit isKindOf "Man") then {
	...
	if  ( (toLower (((_gear select 3) select 0) select 1)) in ["laserdesignator","laserdesignator_02", "Laserdesignator_02_ghex_F", "Laserdesignator_01_khk_F"] && ! isnull (uniformContainer _unit)) then { (uniformContainer _unit) addItemCargoGlobal ["Laserbatteries",1]} ;
